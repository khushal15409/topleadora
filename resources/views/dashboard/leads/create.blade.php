...
@section('content')
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-header bg-transparent border-0 pb-0 pt-4 px-4">
            <h5 class="mb-0">{{ __('Add lead') }}</h5>
            <p class="text-body-secondary small mb-0 mt-1">{{ __('Required fields are marked.') }}</p>
        </div>
        <div class="card-body p-4">
            <form method="post" action="{{ route('dashboard.leads.store') }}">
                @csrf
